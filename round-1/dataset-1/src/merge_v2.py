#!/usr/bin/env python3
"""Simple merge of OWID and WB data."""
import json
...
def main():
    ...
    # Merge OWID data
    logger.info("Merging OWID data...")
    owid = vdem
    for df in [gini, edu, social, gdp, pop, resource]:
        owid = pd.merge(owid, df, on=['Entity', 'Code', 'Year'], how='outer')
    
    logger.info(f"OWID merged shape: {owid.shape}")
    
    # Load WB data
    logger.info("Loading WB data...")
    ...
    available_vars = [v for v in vars_to_keep if v in merged.columns]
